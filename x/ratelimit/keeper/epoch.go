...
func (k Keeper) HasEpochInfo(ctx sdk.Context, identifier string) bool {
	...
	return store.Has(append(types.EpochKeyPrefix, []byte(identifier)...))
}
...
func (k Keeper) GetEpochInfo(ctx sdk.Context, identifier string) types.EpochInfo {
	...
	store := ctx.KVStore(k.storeKey)
	b := store.Get(append(types.EpochKeyPrefix, []byte(identifier)...))
	if b == nil {
		return epoch
	}
...
func (k Keeper) setEpochInfo(ctx sdk.Context, epoch types.EpochInfo) {
	...
	store.Set(append(types.EpochKeyPrefix, []byte(epoch.Identifier)...), value)
}

// DeleteEpochInfo delete epoch info.
func (k Keeper) DeleteEpochInfo(ctx sdk.Context, identifier string) {
	store := ctx.KVStore(k.storeKey)
	store.Delete(append(types.EpochKeyPrefix, []byte(identifier)...))
}
...
func (k Keeper) AfterEpochEnd(ctx sdk.Context, epochInfo types.EpochInfo) {
	if epochInfo.Identifier == types.DayEpoch {
		// Ensure CurrentEpoch is non-negative before converting to uint64
		if epochInfo.CurrentEpoch < 0 {
			panic("negative current epoch not allowed")
		}
